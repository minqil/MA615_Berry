---
title: "Data Analysis for Strawberries"
author: "Minqi Li"
date: "10/20/2020"
output: pdf_document
---
...
# 1 Introduction
## 1.1 Overview
These data were collected from the USDA database selector: <a href="https://quickstats.nass.usda.gov">https://quickstats.nass.usda.gov</a>. We select each variable and get the dataset. This dataset is about the information of three kinds of berries: blueberries, strawberries and raspberries. This report only analyzes the data of strawberries.
...
# 2 Data Cleaning
First of all, I read the data and the data is complicate.
...
sberry1<-read.csv(file="sberry1.csv",header=T)
...
```{r,echo=FALSE, message=FALSE, warning=FALSE,fig.width=6, fig.cap="Plot the histogram with the value measured in $ for each state and each year."}
###MEASURED IN $
...
ggplot(data=measure_mean_state_mea1)+
  ...
  ggtitle("MEASURED IN $")+
  theme(axis.text.x = element_text(angle = 70, hjust = 0.5,vjust = 0.5,color = "black",size=8),plot.title = element_text(hjust = 0.5))

```

```{r,echo=FALSE, message=FALSE, warning=FALSE, fig.width=6, fig.cap="Plot the histogram with the value measured in CWT for each state and each year."}
measure_mean_state_mea2<-filter(measure_mean_state,Measures=="MEASURED IN CWT")
ggplot(data=measure_mean_state_mea2)+
  geom_bar(
    mapping=aes(x=State,y=val1,fill=Year),stat = "identity", position = position_dodge(0.5))+
  ggtitle("MEASURED IN CWT")+
  theme(axis.text.x = element_text(angle = 70, hjust = 0.5,vjust = 0.5,color = "black",size=8),plot.title = element_text(hjust = 0.5))
```

```{r,echo=FALSE, message=FALSE, warning=FALSE, fig.width=6, fig.cap="Plot the histogram with the value measured in CWT/ACRE for each state and each year."}
measure_mean_state_mea3<-filter(measure_mean_state,Measures=="MEASURED IN CWT / ACRE")
ggplot(data=measure_mean_state_mea3)+
  geom_bar(
    mapping=aes(x=State,y=val1,fill=Year),stat = "identity", position = position_dodge(0.5))+
  ggtitle("MEASURED IN CWT/ACRE")+
  theme(axis.text.x = element_text(angle = 70, hjust = 0.5,vjust = 0.5,color = "black",size=8),plot.title = element_text(hjust = 0.5))
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.height=8, fig.width=8, fig.cap="Boxplots separated for each measures."}
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.height=8, fig.width=10, fig.cap="Boxplots separated for each measures."}
